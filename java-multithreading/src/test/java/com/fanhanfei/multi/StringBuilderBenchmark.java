 ...
 *
 * @Warmup
 * 上面我们提到了，进行基准测试前需要进行预热。
 * 一般我们前几次进行程序测试的时候都会比较慢，
 * 所以要让程序进行几轮预热，保证测试的准确性。
 * 其中的参数iterations也就非常好理解了，就是预热轮数。
 *
 * 为什么需要预热？因为 JVM 的 JIT 机制的存在，
 * 如果某个函数被调用多次之后，JVM 会尝试将其编译成为机器码从而提高执行速度。
 ...
 * 关于State的用法，官方的 code sample 里有比较好的例子。http://hg.openjdk.java.net/code-tools/jmh/file/cb9aa824b55a/jmh-samples/src/main/java/org/openjdk/jmh/samples/JMHSample_03_States.java
 **/
